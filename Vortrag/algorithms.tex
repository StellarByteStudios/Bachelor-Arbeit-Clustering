\section{Algorithmen}



\begin{frame}{Algorithmen für fair k-center}
    \begin{minipage}[t]{0.5\textwidth}
        \Copy{AlgorithmenÜbersicht}{
            \begin{itemize}
                \item Vergleich 2 verschiedener Ansätze
                \begin{itemize}
                    \item Red-Clustering \footfullcite{chierichetti2018fair}
                    \item Fast-Anchor-Clustering \footfullcite{BA_Clustering_Irina}
                \end{itemize}
                \item Nutzen von Fairlets
                \begin{itemize}
                    \item kleinst mögliche Clustereinheit
                \end{itemize}
                \item beschränkt auf 2 Farben
                \item beide Algorithmen verallgemeinbar
                \item Aufteilung in 1:1 Cluster
            \end{itemize}
        }
    \end{minipage}
\end{frame}















\begin{frame}{Algorithmen für fair k-center (mit Ausreißern)}
    \begin{minipage}[t]{0.5\textwidth}
        \Paste{AlgorithmenÜbersicht}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.45\textwidth}
        Zusatz: Ausreißer
        \begin{itemize}
            \item reelle Daten nicht perfekt 1:1
            \item so viele Ausreißer, sodass 1:1
            \begin{itemize}
                \item werden nicht mehr in Cluster berücksicthigt
            \end{itemize}
            \item bei $n:n+k$ Verteilung $\rightarrow k$ Ausreißer 
        \end{itemize}
    \end{minipage} 
\end{frame}










% ================= Folie: Was ist unsere Mission ================= %

\begin{frame}{Aufgabe?}
    \begin{LARGE}
        \begin{center}
            \color{hhuBlau}
            \Large Was war die Mission?
        \end{center}
    \end{LARGE} 
\end{frame}


\begin{frame}{Aufgabe?}
    \begin{LARGE}
        \begin{center}
            \color{hhuBlau}
            \Large Was war die Mission?
        \end{center}
    \end{LARGE} 
    \vspace{0.2\textheight}
    \begin{minipage}[t]{0.49\textwidth}
        \begin{center}
            \begin{large}
                Implementierung der Algorithmen
            \end{large}
        \end{center}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.49\textwidth}
        \begin{center}
            \begin{large}
                Vergleich durch Testen der Laufzeit und Güte
            \end{large}
        \end{center}
    \end{minipage} 
\end{frame}























\begin{frame}{Fairlets}
    \begin{itemize}
        \item kleinste, faire Clustereinheit
        \begin{itemize}
            \item hier 1:1 (Rot : Blau)
        \end{itemize}
        \item Nutzen von Fairlets
        \begin{itemize}
            \item Fairlets immer zusammen zu Clustern hinzufügen
            \item $\rightarrow$ Cluster immer ausgeglichen
        \end{itemize}
        \item können optimal berechnet werden über Matching
        \item Unterschied zwischen Red-Clustering und Fast-Anchor-Clustering
    \end{itemize}

\end{frame}













\begin{frame}{Red-Clustering}
    \begin{minipage}[t]{0.55\textwidth}
        \Copy{RedClusteringFluss}{
            \begin{large}
                Finden der Fairlets
            \end{large}
            \begin{enumerate}
                \item bipartiter Graph mit roten und blauen Knoten aufbauen
                \item mit Quelle $s$ und Senke $t$ zum Flussnetzwerk machen
                \item Kanten $s \rightarrow$ Rot und Blau $\rightarrow t$ einfügen
                \item Kanten zwischen den Mengen nach Bedingung:
                \begin{itemize}
                    \item $r_{test} \geq dist(a, b)$ dann Kante
                \end{itemize}
                \item maximalen Fluss berechnen
                \item bestes überdeckendes Matching suchen
                \item gematchte Punkte sind Fairlets
                \item nicht gematchte Punkte $\rightarrow$ Ausreißer
            \end{enumerate}
        }
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.44\textwidth}
        \vspace{-\baselineskip} % Move image to the top
        % Beispiel Flusserstellung
        \includegraphics[width=\textwidth]{fig/Flüsse/Flussbeispiel Fairlettbildung (bipartit).jpg}
    \end{minipage}
\end{frame}


\begin{frame}{Red-Clustering}
    \begin{minipage}[t]{0.55\textwidth}
        \Paste{RedClusteringFluss}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.44\textwidth}
        \vspace{-\baselineskip} % Move image to the top
        % Beispiel Flusserstellung
        \includegraphics[width=\textwidth]{fig/Flüsse/Flussbeispiel Fairlettbildung (Quell und Senk Knoten).jpg}
    \end{minipage}
\end{frame}


\begin{frame}{Red-Clustering}
    \begin{minipage}[t]{0.55\textwidth}
        \Paste{RedClusteringFluss}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.44\textwidth}
        \vspace{-\baselineskip} % Move image to the top
        % Beispiel Flusserstellung
        \includegraphics[width=\textwidth]{fig/Flüsse/Flussbeispiel Fairlettbildung (s-t-Kanten).jpg}
    \end{minipage}
\end{frame}


\begin{frame}{Red-Clustering}
    \begin{minipage}[t]{0.55\textwidth}
        \Paste{RedClusteringFluss}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.44\textwidth}
        \vspace{-\baselineskip} % Move image to the top
        % Beispiel Flusserstellung
        \includegraphics[width=\textwidth]{fig/Flüsse/Flussbeispiel Fairlettbildung (ohne Fluss).jpg}
    \end{minipage}
\end{frame}


\begin{frame}{Red-Clustering}
    \begin{minipage}[t]{0.55\textwidth}
        \Paste{RedClusteringFluss}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.44\textwidth}
        \vspace{-\baselineskip} % Move image to the top
        % Beispiel Flusserstellung
        \includegraphics[width=\textwidth]{fig/Flüsse/Flussbeispiel Fairlettbildung.jpg}
    \end{minipage}
\end{frame}















% ================= Red-Clustering Frames ================= %

\begin{frame}{Red-Clustering}
    \begin{minipage}[t]{0.40\textwidth}
        \Copy{RedClusteringAlgorithm}{
            \begin{large}
                Clustern mit Fairlets
            \end{large}
            \begin{enumerate}
                \item nur rote Punkte clustern
                \begin{itemize}
                    \item mit Gonzalez
                \end{itemize}
                \item alle blauen Punkte zu ihren Partnern zuweisen
                \item Ausreißer bleiben bestehen
            \end{enumerate}
        }
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.55\textwidth}
        \vspace{-\baselineskip} % Move image to the top
        % Beispiel Fertiges Red-Clustering
        \includegraphics[width=\textwidth]{fig/Red-Clustering/Red-Clustering Ausgangslage.jpg}
    \end{minipage}
\end{frame}


\begin{frame}{Red-Clustering}
    \begin{minipage}[t]{0.40\textwidth}
        \Paste{RedClusteringAlgorithm}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.55\textwidth}
        \vspace{-\baselineskip} % Move image to the top
        % Beispiel Fertiges Red-Clustering
        \includegraphics[width=\textwidth]{fig/Red-Clustering/Red-Clustering nur Rote.jpg}
    \end{minipage}
\end{frame}


\begin{frame}{Red-Clustering}
    \begin{minipage}[t]{0.4\textwidth}
        \Paste{RedClusteringAlgorithm}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.55\textwidth}
        \vspace{-\baselineskip} % Move image to the top
        % Beispiel Fertiges Red-Clustering
        \includegraphics[width=\textwidth]{fig/Red-Clustering/Red-Clustering nach Gonzalez.jpg}
    \end{minipage}
\end{frame}


\begin{frame}{Red-Clustering}
    \begin{minipage}[t]{0.4\textwidth}
        \Paste{RedClusteringAlgorithm}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.55\textwidth}
        \vspace{-\baselineskip} % Move image to the top
        % Beispiel Fertiges Red-Clustering
        \includegraphics[width=\textwidth]{fig/Red-Clustering/Red-Clustering nach Gonzalez mit Blauzuweißung.jpg}
    \end{minipage}
\end{frame}
































\begin{frame}{Fast-Anchor-Clustering}
    \begin{minipage}[t]{0.55\textwidth}
        \begin{large}
            Finden der Fairlets - Anchor Variante
        \end{large}
        \begin{enumerate}
            \item Anker für jedes Knotenpaar bestimmen
            \begin{itemize}
                \item $\rightarrow$ max Abstand der Partner zum Anker minimal
            \end{itemize}
            \begin{scriptsize}
                \item bipartiter Graph mit roten und blauen Knoten aufbauen
                \item mit Quelle $s$ und Senke $t$ zum Flussnetzwerk machen
                \item Kanten $s \rightarrow$ Rot und Blau $\rightarrow t$ einfügen
            \end{scriptsize}
            
            \item Kanten zwischen den Mengen nach Bedingung:
            \begin{itemize}
                \item $r_{test} \geq anchor\text{-}dist(a, b)$ dann Kante
            \end{itemize}
            \begin{scriptsize}
                \item maximalen Fluss berechnen
                \item bestes überdeckendes Matching suchen
                \item gematchte Punkte sind Fairlets
                \item nicht gematchte Punkte $\rightarrow$ Ausreißer
            \end{scriptsize}
            
        \end{enumerate}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.44\textwidth}
        \vspace{-\baselineskip} % Move image to the top
        % Beispiel Flusserstellung
        \includegraphics[width=\textwidth]{fig/Flüsse/Flussbeispiel Fairlettbildung.jpg}
    \end{minipage}
\end{frame}


















% ================= Fast-Anchor Frames ================= %

\begin{frame}{Fast-Anchor-Clustering}
    \begin{minipage}[t]{0.44\textwidth}
        \Copy{FastAnchorAlgorithmus}{
            \begin{large}
                Clustern mit Fairlet-Anker
            \end{large}
            \begin{enumerate}
                \item alle Punkte ohne Ausreißer clustern
                \item Zentren und ihre Fairletpartner sich selbst zuweisen
                \begin{itemize}
                    \item $\rightarrow$ Center-Awareness
                \end{itemize}
                \item Punkte dem nächsten Zentrum ihres Ankers zuweisen
            \end{enumerate}
        }
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.55\textwidth}
        \vspace{-\baselineskip} % Move image to the top
        % Beispiel fertiges Fast-Anchor-Clustering
        \includegraphics[width=\textwidth*6/7]{fig/Fast-Anchor Clustering/Fast-Anchor ohne Ausreisermarkierung.jpg}
    \end{minipage}
\end{frame}



\begin{frame}{Fast-Anchor-Clustering}
    \begin{minipage}[t]{0.44\textwidth}
        \Paste{FastAnchorAlgorithmus}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.55\textwidth}
        \vspace{-\baselineskip} % Move image to the top
        % Beispiel fertiges Fast-Anchor-Clustering
        \includegraphics[width=\textwidth*6/7]{fig/Fast-Anchor Clustering/Fast-Anchor Ausgangslage.jpg}
    \end{minipage}
\end{frame}


\begin{frame}{Fast-Anchor-Clustering}
    \begin{minipage}[t]{0.44\textwidth}
        \Paste{FastAnchorAlgorithmus}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.55\textwidth}
        \vspace{-\baselineskip} % Move image to the top
        % Beispiel fertiges Fast-Anchor-Clustering
        \includegraphics[width=\textwidth*6/7]{fig/Fast-Anchor Clustering/Fast-Anchor mit Fairlettmarkierung.jpg}
    \end{minipage}
\end{frame}


\begin{frame}{Fast-Anchor-Clustering}
    \begin{minipage}[t]{0.44\textwidth}
        \Paste{FastAnchorAlgorithmus}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.55\textwidth}
        \vspace{-\baselineskip} % Move image to the top
        % Beispiel fertiges Fast-Anchor-Clustering
        \includegraphics[width=\textwidth*6/7]{fig/Fast-Anchor Clustering/Fast-Anchor nach Gonzalez.jpg}
    \end{minipage}
\end{frame}


\begin{frame}{Fast-Anchor-Clustering}
    \begin{minipage}[t]{0.44\textwidth}
        \Paste{FastAnchorAlgorithmus}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.55\textwidth}
        \vspace{-\baselineskip} % Move image to the top
        % Beispiel fertiges Fast-Anchor-Clustering
        \includegraphics[width=\textwidth*6/7]{fig/Fast-Anchor Clustering/Fast-Anchor nach Clusterzuweisung.jpg}
    \end{minipage}
\end{frame}


\begin{frame}{Fast-Anchor-Clustering}
    \begin{minipage}[t]{0.44\textwidth}
        \Paste{FastAnchorAlgorithmus}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.55\textwidth}
        \vspace{-\baselineskip} % Move image to the top
        % Beispiel fertiges Fast-Anchor-Clustering
        \includegraphics[width=\textwidth*6/7]{fig/Fast-Anchor Clustering/Fast-Anchor mit Clusteranzeige.jpg}
    \end{minipage}
\end{frame}















\begin{frame}{Red-Clustering vs Fast-Anchor-Clustering (Theorie)}
    \begin{itemize}
        \item Red-Clustering hat Appoximationsgarantie von 4
        \item Fast-Anchor-Clustering hat Appoximationsgarantie von 3
        \item Laufzeit beider Algorithmen $\in \mathcal{O}(n^3)$
    \end{itemize}
\end{frame}

